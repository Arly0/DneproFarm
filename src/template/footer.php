    <footer class="footer" id="footer">
        <div class="container">
            <div class="row">
                <div class="col-lg-2 showDesk">
                    <div class="footer_copy">
                        <img src="images/footer_logo.png" alt="DNEPROFARM">

                        <p>
                            © DNEPROFARM 2019 <br>
                            Всі права захищені
                        </p>
                    </div>
                </div>
                <div class="col-lg-1"></div>
                <div class="col-lg-3">
                    <div class="footer_info">
                        <h4>
                            Головна інформація
                        </h4>
                        <div class="d-flex info">
                            <a href="#"><p>Про компанію</p></a>
                            <a href="#"><p>Партнери</p></a>
                        </div>
                        <div class="d-flex info">
                            <a href="#"><p>Послуги</p></a>
                            <a href="#"><p>Новини</p></a>
                        </div>
                        <div class="d-flex info">
                            <a href="#"><p>Тест-набори</p></a>
                            <a href="#"><p>Контакти</p></a>
                        </div>
                        <div class="d-flex info">
                            <a href="#"><p>Видаткові матеріали</p></a>
                        </div>
                    </div>
                </div>
                <div class="col-lg-1"></div>
                <div class="col-lg-5">
                    <div class="footer_social d-flex">
                        <div class="phones">
                            <h4>
                                Зв’язок з нами
                            </h4>
                            <div class="contacts">
                            <div class="contact d-flex">
                                    <div class="svg_container">
                                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="17" height="16" viewBox="0 0 17 16">
                                        <metadata>
                                        <x:xmpmeta xmlns:x="adobe:ns:meta/" x:xmptk="Adobe XMP Core 5.6-c142 79.160924, 2017/07/13-01:06:39        ">
                                        <rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
                                            <rdf:Description rdf:about=""/>
                                        </rdf:RDF>
                                        </x:xmpmeta></metadata>
                                        <image id="Шар_10" data-name="Шар 10" width="17" height="16" xlink:href="data:img/png;base64,"/>
                                        </svg>

                                    </div>
                                    <div class="phone">
                                        <a href="tel:<?php the_field('info_tel_1') ?>"><?php the_field('info_tel_1') ?></a>
                                    </div>
                                </div>

                                <div class="contact d-flex">
                                    <div class="svg_container">
                                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="17" height="16" viewBox="0 0 17 16">
                                        <metadata>
                                        <x:xmpmeta xmlns:x="adobe:ns:meta/" x:xmptk="Adobe XMP Core 5.6-c142 79.160924, 2017/07/13-01:06:39        ">
                                        <rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
                                            <rdf:Description rdf:about=""/>
                                        </rdf:RDF>
                                        </x:xmpmeta></metadata>
                                        <image id="Шар_10" data-name="Шар 10" width="17" height="16" xlink:href="data:img/png;base64,"/>
                                        </svg>

                                    </div>
                                    <div class="phone">
                                        <?php
                                        if(get_field('info_tel_2')){
                                        ?>
                                        <a href="tel:<?php the_field('info_tel_2') ?>"> <?php the_field('info_tel_2') ?></a>
                                        <?php } ?>
                                        
                                    </div>
                                </div>
                                </div>
                        </div>

                        <div class="social showDesk">
                            <h4>
                                Ми у соц. мережах
                            </h4>
                            <a href="facebook.com"><img src="images/facebook.png" alt="Ми у Facebook"></a>
                        </div>
                    </div>

                    <form action="file.php" method="post">
                        <div class="footer_send d-flex">
                            <input type="email" name="email" class="footer__field" id="" required placeholder="Введіть email">
                            <input type="submit" name="sbm" class="footer__sbm" value="підписатись">
                        </div>
                    </form>

                        <div class="social showMob">
                            <h4>
                                Ми у соц. мережах
                            </h4>
                            <a href="facebook.com"><img src="images/facebook.png" alt="Ми у Facebook"></a>
                        </div>


                    <div class="footer_copy showMob">

                        <p>
                            © DNEPROFARM 2019
                            Всі права захищені
                        </p>
                    </div>
                </div>
            </div>
        </div>
    </footer>
    <script src="js/jquery-3.3.1.min.js"></script>
    <script src="js/vue.js"></script>
    <script src="<?php echo get_template_directory_uri() ?>/js/libs.min.js"></script>
    <script src="js/slicky.min.js"></script>
    <script src="js/scripts.min.js"></script>

    <?php
    if(is_front_page()){

        // * получаем значения трех изображений из админки и подставляем их в background
            $rows = get_field('df_slider');
            $row = $rows[0];
            $first_image = $row['df_slider_image'];

            $row = $rows[1];
            $second_image = $row['df_slider_image'];

            $row = $rows[2];
            $third_image = $row['df_slider_image'];

            $image = wp_get_attachment_image_src($first_image, 'full')
        ?>

        <script>
            $(document).ready(function(){
                $('.partners').find('.slick-active :eq(2)').addClass('slick-center');

                $('.slick_company').mouseup(function(){
                    $('.slick-center').removeClass('slick-center');
                    $('.partners').find('.slick-active :eq(2)').addClass('slick-center');
                });

                $('.slick-dots button').focusin(function(){
                    $('.slick-center').removeClass('slick-center');
                    $('.partners').find('.slick-active :eq(2)').addClass('slick-center');
                });
                

                $('.back-img').attr('style', 'background: url(<?php echo $first_image["url"] ?>)');
                $('.slick-dots button').click(function(){
                    let i = $('.slick-dots button').index($(this));
                    switch(i){
                        case 0:
                            $('.back-img').attr('style', 'background: url(<?php echo $first_image["url"] ?>)');
                            break;
                        case 1:
                            $('.back-img').attr('style', 'background: url(<?php echo $second_image["url"] ?>)');
                            break;
                        case 2:
                            $('.back-img').attr('style', 'background: url(<?php echo $third_image["url"] ?>)');
                            break;
                    }
                });
            });
        </script>
    <?php
    }
    ?>
</body>
</html>